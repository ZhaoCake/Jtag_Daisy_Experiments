...
class IbexSystem(idcode: Int) extends Module {
  ...
  val ram_sel_sba = sba_req
  val ram_sel_core = core_req_ram && !sba_req
  val ram_sel_instr = instr_req_ram && !core_req_ram && !sba_req
  ...
  val ram_rvalid = RegNext(ram_req && !ram_we)
  ...
  dm.io.master_r_valid_i := ram_resp_to_sba && ram_rvalid
